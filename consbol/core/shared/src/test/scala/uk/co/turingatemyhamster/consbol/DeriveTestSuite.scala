...
object DeriveTestSuite extends TestSuite {
  ...
  val tests = TestSuite {
    ...
    implicit class DSOps[M](_ds0: DerivationState[M]) {
      def |- [A](a: A)(implicit d: Derive[A, M]) =
...
//
//          'kab - assert(k_ab.head.value.goal == LT_EQ('a, 'b))
//          'kbc - assert(k_bc.head.value.goal == LT_EQ('b, 'c))
//
